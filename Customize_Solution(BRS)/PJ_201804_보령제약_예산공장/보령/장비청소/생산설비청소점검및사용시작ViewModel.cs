...
namespace 보령
{
    public class 생산설비청소점검및사용시작ViewModel : ViewModelBase
    {
        #region [Property]
        public 생산설비청소점검및사용시작ViewModel()
        {
            ...
            _BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT = new BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT();
            _ListContainer = new ObservableCollection<보령.생산설비청소점검및사용시작ViewModel.ComboList>()
            {
        ...
        private 생산설비청소점검및사용시작 _mainWnd;
        ...
        private BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT _BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT;
        public BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT
        {
            get { return _BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT; }
            set
            {
                _BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT = value;
                OnPropertyChanged("BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT");
            }
        }
        #endregion
        #region [Command]
                ...
                return new AsyncCommandBase(async arg =>
                {
                    using (await AwaitableLocks["LoadCommandAsync"].EnterAsync())
                    {
                        try
                        {
                            ...
                            if (arg != null && arg is 생산설비청소점검및사용시작)
                            {
                                this._mainWnd = arg as 생산설비청소점검및사용시작;
                                CANRECORDFLAG = false;
                    ...
                    using (await AwaitableLocks["ConfirmCommandAsync"].EnterAsync())
                    {
                        try
                        {
                            ...
                            if (BR_BRS_SEL_EquipmentStatus_PROCEQPT.OUTDATAs.Count > 0)
                            {
                                ...
                                /* 2025.08.18 조영호 생산설비청소점검 기록 시 생산설비 사용 시작을 위한 사용 시작 로직 가져옴                                 
                                 */
                                _BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT.INDATAs.Clear();
                                if (refInst.Count > 0)
                                {
                                    foreach (InstructionModel Inst in refInst)
                                    {
                                        ...
                                        if (Inst.Raw.ACTVAL == null)
                                        {
                                            if (Inst.Raw.ACTVAL != "N/A")
                                            {
                                                _BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT.INDATAs.Add(new BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT.INDATA
                                                {
                                                    ROOMNO = AuthRepositoryViewModel.Instance.RoomID,
                                                ...
                                                _BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT.PARAMDATAs.Add(new BR_BRS_UPD_EquipmentAction_ShopFloor_PROCSTRT_PROCEQPT.PARAMDATA
                                                {
                                                    EQPAID = LGCNS.iPharmMES.Common.AuthRepositoryViewModel.GetSystemOptionValue("LOGBOOK_PRODUCTION_ORDER"),
                                                    PAVAL = _mainWnd.CurrentOrder.ProductionOrderID,
                                                    ROOMNO = AuthRepositoryViewModel.Instance.RoomID
                                                });
