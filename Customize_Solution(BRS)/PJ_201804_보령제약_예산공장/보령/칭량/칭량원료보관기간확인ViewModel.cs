...
namespace 보령
{
    public class 칭량원료보관기간확인ViewModel : ViewModelBase
    {
        #region [Property]
        public 칭량원료보관기간확인ViewModel()
        {
            _BR_BRS_SEL_DispensingExpireState = new BR_BRS_SEL_DispensingExpireState();
        }
        ...
        private BR_BRS_SEL_DispensingExpireState _BR_BRS_SEL_DispensingExpireState;
        public BR_BRS_SEL_DispensingExpireState BR_BRS_SEL_DispensingExpireState
        {
            get { return _BR_BRS_SEL_DispensingExpireState; }
            set
            {
                _BR_BRS_SEL_DispensingExpireState = value;
                OnPropertyChanged("BR_BRS_SEL_DispensingExpireState ");
            }
        }
                ...
                return new AsyncCommandBase(async arg =>
                {
                    using (await AwaitableLocks["ConfirmCommand"].EnterAsync())
                    {
                        try
                        {
                            ...
                            if (_BR_BRS_SEL_DispensingExpireState.OUTDATAs.Count == 0)
                            {
                            ...
                            if (_BR_BRS_SEL_DispensingExpireState.OUTDATAs.Count > 0)
                            {
                                foreach (var item in _BR_BRS_SEL_DispensingExpireState.OUTDATAs)
                                {
