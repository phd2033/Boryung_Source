...
namespace 보령
{
    ...
    public class 피마과립무게측정ViewModel : ViewModelBase
    {
        #region [Property]
        public 피마과립무게측정ViewModel()
        {
            ...
            get
            {
                return new AsyncCommandBase(async arg =>
                {
                    try
                    {
                        ...
                        if (arg != null && arg is 피마과립무게측정)
                        {
                            _mainWnd = arg as 피마과립무게측정;
                            _mainWnd.Closed += (s, e) =>
                            {
                                if (_repeater != null)
                                    _repeater.Stop();
                    ...
                    try
                    {
                        ...
                        if (await _BR_BRS_SEL_VESSEL_Info.Execute())
                        {
                            if (CheckVesselId(VesselId))
                            {
                                ...
                                _MsubLotId = _BR_BRS_SEL_VESSEL_Info.OUTDATAs[0].MSUBLOTID;
                                _repeater.Start();
                                OnPropertyChanged("TotalWeight");
                            }
