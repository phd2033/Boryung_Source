                ...
                return new AsyncCommandBase(async arg =>
                {
                    using (await AwaitableLocks["AddInterferCommandAsync"].EnterAsync())
                    {
                        try
                        {
                            ...
                            else if (CONTENTS != "환경모니터링(Envionmental Monitoring)" & (MODULE.Equals("1~3 Module") || MODULE.Equals("1~5 Module"))) { throw new Exception("1~3 Module과 1~5 Module은 [ 환경모니터링 ] 간섭내용에서만 사용합니다.\n간섭내용과 Module이 맞는지 확인해주세요."); }
