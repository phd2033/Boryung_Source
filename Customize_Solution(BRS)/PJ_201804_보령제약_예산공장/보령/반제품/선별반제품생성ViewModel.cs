...
namespace 보령
{
    public class 선별반제품생성ViewModel : ViewModelBase
    {

        선별반제품생성ViewModel _parentVM;
        public 선별반제품생성ViewModel ParentVM
        {
                ...
                return new AsyncCommandBase(async arg =>
                {
                    using (await AwaitableLocks["LoadCommand"].EnterAsync())
                    {
                        try
                        {
                            ...
                            ///
                            if (arg != null) _mainWnd = arg as 선별반제품생성;
        ...
        // 선별 반제품으로 생성
        public ICommand CreateCommandAsync
        {
            get
            {
                return new AsyncCommandBase(async arg =>
                {
                    using (await AwaitableLocks["CreateCommandAsync"].EnterAsync())
                    {
                        try
                        {
                            IsBusy = true;

                            CommandResults["CreateCommandAsync"] = false;
                            CommandCanExecutes["CreateCommandAsync"] = false;
                            ...
                            CommandResults["CreateCommandAsync"] = true;
                        }
                        catch (Exception ex)
                        {
                            CommandResults["CreateCommandAsync"] = false;
                        ...
                        {
                            CommandCanExecutes["CreateCommandAsync"] = true;
                ...
                {
                    return CommandCanExecutes.ContainsKey("CreateCommandAsync") ?
                        CommandCanExecutes["CreateCommandAsync"] : (CommandCanExecutes["CreateCommandAsync"] = true);
                });
            }
        }
